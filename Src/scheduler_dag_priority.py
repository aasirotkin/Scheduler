...
# планировщик по приоритетам: запускает задачу только, если у нее не осталось зависимостей + раньше берёт задачу с наивысшим приоритетом
class DagUserPriorityScheduler(BaseScheduler):
    ...
    # сравниваем готовые задачи. чем больше пользовательский priority, тем раньше задача будет выбрана
    def _ready_tuple(self, tid: str, tasks: Dict[str, TaskSpec]) -> Tuple:
        return (-tasks[tid].priority, id_sort_key(tid))
    ...
    # цикл планировщика по приоритетам: ready-задачи сортируются по priority,  после завершения успешной задачи закрываем зависимости у ее потомков
    def run(self, runners: List, tasks: Iterable) -> Dict[str, TaskResult]:
        if not runners:
            raise ValueError("Priority scheduler requires at least one runner")
