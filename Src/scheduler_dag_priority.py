...
# планировщик по приоритетам: запускает задачу только, если у нее не осталось зависимостей
# + раньше берёт задачу с наивысшим приоритетом
class DagUserPriorityScheduler(BaseScheduler):
    ...
    # сравниваем готовые задачи. чем больше пользовательский priority,
    # тем раньше задача будет выбрана
    ...
    # цикл планировщика по приоритетам: ready-задачи сортируются по priority,
    # после завершения успешной задачи закрываем зависимости у ее потомков
