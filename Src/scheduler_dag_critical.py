...
# планировщик по критическому пути: раньше запускаем те задачи, которые сильнее всего влияют на общее время выполнения.
# из готовых задач берем по коэффициенту blevel + сортировка по приоритету
class DagCriticalPathScheduler(BaseScheduler):
    ...
    # запуск 1 задачи,  теперь сам запуск находится в runner.exec(task)
    ...
    # строим топологический граф, считаем коэффициенты blevel: потомков раньше родителей
    ...
    # сравниваем готовые задачи: больше b-level -> раньше запуск + приоритеты, если равны коэффициенты
    ...
    # цикл critical-path scheduler: считаем blevel, потом выбираем из ready задачи
    # с самым длинным критическим хвостом
    def run(self, runners: List, tasks: Iterable) -> Dict[str, TaskResult]:
        if not runners:
            raise ValueError("Critical-path scheduler requires at least one runner")
        ...
        self._set_running()
        try:
            blevel = self._compute_blevel(task_specs)

            return self._run_with_ready_queue(
                runners=runners,
                task_objects=task_objects,
                tasks=task_specs,
                ready_key_builder=lambda tid: self._ready_tuple(tid, task_specs, blevel),
            )
        finally:
            self._set_finished()
