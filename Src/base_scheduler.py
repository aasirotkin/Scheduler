...
from Include.Scheduler.IFaces.i_scheduler import IScheduler, SchedulerStatus
from Include.Scheduler.IFaces.i_task import ITask
from Include.Scheduler.IFaces.task_result import TaskResult


# общая база для всех планировщиков, здесь остаются параметры запуска,
# хранилище добавленных задач и результатов, базовые методы add/remove/list/get_result/get_results
class BaseScheduler(IScheduler):
    def __init__(
        ...
        *,
        ...
        # статус планировщика: работает или уже закончил
        self._status = SchedulerStatus.FINISHED

    def get_status(self) -> SchedulerStatus:
        return self._status

    # перевести планировщик в состояние RUNNING
    def _set_running(self) -> None:
        self._status = SchedulerStatus.RUNNING

    # перевести планировщик в состояние FINISHED
